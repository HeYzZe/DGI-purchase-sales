<template>
  <el-container>
    <el-header
      ...
      <router-link :to="{ name: 'OutStorageInfo' }">
        <el-button
          size="medium"
          icon="el-icon-plus"
          round
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.$http.post('dgi/dgiSale', {
        psId: this.commonModel.psId,
        operator: this.itemList[0].operator,
        saleContract: this.commonModel.saleContract,
        totalAmount: this.commonModel.totalAmount,
        ...
        this.$store.commit('setOutStorageList', []);
    ...
    onDelete() {
      this.itemList.filter(n => !n.isSelected);
      this.$dd.confirm({
        title: '提示',
        message: '是否确认删除？',
      }, (index) => {
        if (index === 1) {
          this.$store.commit('setOutStorageList', { ...this.itemList.filter(n => !n.isSelected) });
          this.isEdit = false;
        }
      });
    },
