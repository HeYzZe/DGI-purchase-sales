<template>
  <el-container>
    <el-header
      ...
      <router-link :to="{ name: 'DgiTestInfo' }">
        <el-button
          size="medium"
          icon="el-icon-plus"
          round
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.$http.post('dgi/dgiTest', this.itemList).then((response) => {
        ...
        this.$store.commit('setTestDgiInfo', []);
    ...
    onDelete() {
      this.$dd.confirm({
        ...
        if (index === 1) {
          this.$store.commit('setTestDgiInfo', { ...this.itemList.filter(n => !n.isSelected) });
          this.isEdit = false;
        }
      });
    },
