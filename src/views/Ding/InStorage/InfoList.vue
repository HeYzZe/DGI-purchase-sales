...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.$http.post('dgi/dgiInfo', this.itemList).then((response) => {
        ...
        this.$store.commit('deleteDgiInfo', []);
    ...
    onDelete() {
      ...
      this.$dd.confirm({
        ...
        if (index === 1) {
          this.$store.commit('deleteDgiInfo', { ...this.itemList.filter(n => !n.isSelected) });
          this.isEdit = false;
        }
      });
    },
